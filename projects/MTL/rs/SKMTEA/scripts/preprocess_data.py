...
def main(args):
    ...
    save_data_path = Path(args.annotations_path).parent.parent / data_type
    seg_data_path = Path(args.data_path).parent /seg_type
    raw_data_path = Path(args.data_path)
    crop_scale = args.crop_scale


    seg_save_data_path = Path(args.annotations_path).parent.parent /seg_type
    ...
    for annotation_set in annotations_sets:
        ...
        for image in tqdm(annotation_set["images"]):
            rfname = f'{raw_data_path}/{image["file_name"]}'
            wfname = f'{save_data_path}/{image["file_name"]}'
            segrfname = f'{seg_data_path}/{image["file_name"]}'.replace('.h5','.nii.gz')
            segwfname = f'{seg_save_data_path}/{image["file_name"]}'.replace('.h5', '.nii.gz')
            meta_dir = raw_data_path.parent / 'all_metadata.csv'
            meta_dataframe = pd.read_csv(meta_dir,index_col=0)
            meta_data =meta_dataframe[meta_dataframe['MTR_ID']==image["file_name"].replace('.h5','')]
            ...
            target_shape = np.array(
                [z_new, x_new, y_new, segmentation_one.shape[3]], int)
