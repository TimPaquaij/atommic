    ...
    for annotation_set_path in annotations_sets:
        ...
        for image in tqdm(range(len(annotation_set["images"]))):
            if annotation_set["images"][image]["file_name"] == "MTR_135.h5":
                ...
                pixelsize_y_new = pixelsize_y_old * (1 / (kspace.shape[2] / kspace_old[1])) * (1 / crop_scale[1])
                slice_thickness_new = slice_thickness_old * (1 / (kspace.shape[0] / kspace_old[2])) * (1 / crop_scale[2])
                ...
                for annotation in range(len(annotation_set['annotations'])):
                    if annotation_set['annotations'][annotation]['image_id'] == annotation_set["images"][image]['id']:
                        annotation_set['annotations'][annotation]['bbox'] = [int((kspace.shape[0]/kspace_old.shape[2])*annotation_set['annotations'][annotation]['bbox'][2]),int((kspace.shape[1]/kspace_old.shape[0])*annotation_set['annotations'][annotation]['bbox'][0]),int(((kspace.shape[2])/kspace_old.shape[1])*annotation_set['annotations'][annotation]['bbox'][1]),int((kspace.shape[0]/kspace_old.shape[2])*annotation_set['annotations'][annotation]['bbox'][5]),int(crop_scale[0]*annotation_set['annotations'][annotation]['bbox'][3]),int(((512-96)/512)*annotation_set['annotations'][annotation]['bbox'][4]*crop_scale[1])]
